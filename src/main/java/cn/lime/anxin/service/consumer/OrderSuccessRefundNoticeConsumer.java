...
/**
 * @ClassName: OrderSuccessRefundNoticeConsumer
 * @Description: TODO
 * @Author: Lime
 * @Date: 2024/8/23 15:54
 */
@Service
@Slf4j
public class OrderSuccessRefundNoticeConsumer {
    @Resource
    ...
    public void consumer(){
        String orderIdStr = redisTemplateMap.get(RedisDb.BIZ_DB.getVal()).opsForList()
                .leftPop(RedisKeyName.BIZ_NOTICE_SUCCESS_REFUND_ORDER.getVal());
        if (StringUtils.isNotEmpty(orderIdStr)) {
            try {
                ...
                redisTemplateMap.get(RedisDb.BIZ_DB.getVal()).opsForList()
                        .rightPush(RedisKeyName.BIZ_NOTICE_SUCCESS_REFUND_ORDER.getVal(),orderIdStr);
            }
        }
    }
